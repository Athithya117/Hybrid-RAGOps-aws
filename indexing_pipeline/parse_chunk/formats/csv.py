...
TARGET_TOKENS_PER_CHUNK = int(os.getenv("CSV_TARGET_TOKENS_PER_CHUNK", "2000"))
ROWS_PER_CHUNK_OVERRIDE = os.getenv("CSV_ROWS_PER_CHUNK", "")
MIN_ROWS_PER_CHUNK = int(os.getenv("CSV_MIN_ROWS_PER_CHUNK", "1"))
MAX_ROWS_PER_CHUNK = int(os.getenv("CSV_MAX_ROWS_PER_CHUNK", "1000"))
...
def _flush_rows_chunk(
    ...
    log.info(f"Wrote CSV chunk {payload['chunk_id']} → {out_key}")
    ...
    for _, row in rows_iterable:
        ...
        if row_tokens > TARGET_TOKENS_PER_CHUNK:
            ...
            for w in windows:
                ...
                body = ((json.dumps(payload, ensure_ascii=False) + "\n").encode() if ext == "jsonl" else json.dumps(payload, indent=2, ensure_ascii=False).encode())
                s3_put_object_with_retries(out_key, body, content_type="application/json")
                log.info(f"Wrote CSV token window {payload['chunk_id']} → {out_key}")
                saved += 1
            start_row_of_current = next_row_num
            continue
