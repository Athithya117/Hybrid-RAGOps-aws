...
def is_garbage_line(line: str) -> bool:
    ...
    if line.startswith("Fig.") or line.startswith("Table "):
        return True
    if re.match(r"^[0-9\.]+\s+\w+", line):
        return True
    if re.match(r"^(dp_redis_density|k0s|k3s|k8s|OpenYurt|KubeEdge)\b", line):
        return True
    if re.search(r"^[\[\]\{\}<>]", line):
        return True
    # drop stray chart fragments like '_Censity' or repeated labels
    if "_Censity" in line or "ram ram disk" in line:
        return True
    return False
...
def parse_file(s3_key: str, manifest: dict) -> dict:
    ...
    for idx, page in enumerate(mp):
        ...
        figures = []  # collect image OCR separately
        ...
        for i, tbl in enumerate(raw_tables, 1):
            ...
            if is_valid_table(norm):
                ...
                txt = "\n".join("\t".join(r) for r in norm if any(r))
                table_items.append((0.0, f"[TABLE_{i}]\n{txt}"))

        # 5) Merge & clean with garbage filter
        merged = line_items + img_items + table_items
        merged.sort(key=lambda x: x[0])
        raw_lines = [l for _, l in merged]
        clean = [
            ln for ln in raw_lines
            if not is_garbage_line(ln) and is_ocr_line_valid(ln)
        ]
        ...
        # 7) Upload and log time per page
...
# --- If run as script, process all PDFs under raw prefix ---
