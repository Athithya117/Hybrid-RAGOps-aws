...
if OCR_BACKEND not in ("rapidocr", "tesseract"):
    print(f"ERROR: OCR_ENGINE must be 'rapidocr' or 'tesseract', got '{OCR_BACKEND}'", file=sys.stderr)
    sys.exit(1)
...
if IS_MULTILINGUAL and OCR_BACKEND == "rapidocr":
    print("ERROR: multilingual OCR not supported with rapidocr. Set OCR_ENGINE=tesseract when IS_MULTILINGUAL=true", file=sys.stderr)
    sys.exit(1)
...
# Build robust multilingual config for Tesseract
...
# Core Tesseract flags
TESS_CONFIG = (
    f"-l {TESS_LANGS} "
    "--oem 1 "                         # LSTM engine only
    "--psm 3 "                         # automatic page segmentation
    "-c preserve_interword_spaces=1 "  # preserve spaces
    "-c language_model_penalty_non_dict_word=0.5 "  # penalize non-dictionary words
    "-c load_freq_dawg=F "             # disable frequency DAWG
    "-c load_system_dawg=F"            # disable system DAWG
)
...
# --- If using RapidOCR, lazy load here ---
...
def parse_file(s3_key: str, manifest: dict) -> dict:
    ...
    for idx, page in enumerate(mp_doc):
        ...
        # Merge native text and OCR output
        ...
        for i, info in enumerate(srcs, start=1):
            try:
                ...
                # Append image OCR results to text only
                block = "\n".join(lines)
                payload["text"] += f"\n[IMG_OCR:{i}]\n{block}"
