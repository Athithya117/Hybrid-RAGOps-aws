...
# Ensure local package path resolution (keeps parity with other frontend modules)
ROOT_DIR = os.path.dirname(os.path.abspath(__file__)) if "__file__" in globals() else None
if ROOT_DIR and ROOT_DIR not in __import__("sys").path:
    __import__("sys").path.insert(0, ROOT_DIR)

# Use the structured, single-line JSON logger used elsewhere
from logger import log

# --- configuration via env ---
JWT_SECRET = os.getenv("JWT_SECRET") or ""
if not JWT_SECRET:
    # preserve previous behavior but structured
    log.warn("JWT_SECRET not set; generating ephemeral secret (NOT for production).")
...
# provider/client envs & other config come from config module normally;
# file expects functions get_redirect, enabled_providers_effective, etc.
from config import (
    get_redirect,
    enabled_flags,
...
# feature flags and provider list
flags = enabled_flags()
effective = enabled_providers_effective()

if any(flags.values()) and not effective:
    log.info("Provider flags set but no provider had both CLIENT_ID and CLIENT_SECRET. Providers will remain disabled until credentials provided.", flags=list(flags.keys()))

# small inline svgs used by the UI (kept unchanged)
...
if "google" in effective:
    ...
    log.info("oauth.provider.registered", provider="google")
if "microsoft" in effective:
    ...
    log.info("oauth.provider.registered", provider="microsoft", tenant=ms_tenant)
if "github" in effective:
    ...
    log.info("oauth.provider.registered", provider="github")
...
async def login_page():
    ...
    if not providers:
        ...
        for p in providers:
            btns.append(
                "<a href='/auth/login/start/{p}' class='w-full inline-flex items-center justify-center border rounded py-2 px-3 mb-3' aria-label='Continue with {cap}'>{svg}<span style='margin-left:8px'>Continue with {cap}</span></a>".format(
                    p=p, cap=p.capitalize(), svg=icons.get(p, "")
                )
            )
        btns_html = "\n".join(btns)
...
async def login_start(request: Request, provider: str):
    ...
    try:
        ...
        log.error("OAuth redirect initiation failed", provider=provider, stack=tb)
...
async def _fetch_userinfo_with_token(provider: str, access_token: str, client: Any = None) -> Dict[str, Any]:
    ...
    try:
        ...
        log.warn("userinfo fetch failed", provider=provider, error=str(e), stack=tb)
    return {}
...
async def callback(request: Request, provider: str):
    ...
    try:
        ...
        log.warn("InvalidClaimError during authorize_access_token; will attempt manual exchange", provider=provider, error=str(err))
        token = None
    except OAuthError as err:
        log.error("OAuthError during authorize_access_token", provider=provider, error=str(err))
        token = None
    except Exception as err:
        tb = "".join(traceback.format_exception(type(err), err, err.__traceback__))
        log.error("authorize_access_token failed unexpectedly", provider=provider, stack=tb)
        token = None
    ...
    if not token or (isinstance(token, dict) and not token.get("access_token") and not token.get("id_token")):
        ...
        if not token_endpoint:
            log.error("No token endpoint available for manual exchange", provider=provider)
            return RedirectResponse(url=f"{get_redirect(provider).rsplit('/auth',1)[0]}/auth/success?error=oauth", status_code=302)
        try:
            async with httpx.AsyncClient(timeout=15.0) as h:
                ...
                if isinstance(token, dict):
                    log.debug("Manual token exchange result keys", keys=list(token.keys()))
        except Exception as e:
            tb = "".join(traceback.format_exception(type(e), e, e.__traceback__))
            log.error("Manual token exchange for provider failed", provider=provider, stack=tb)
            return RedirectResponse(url=f"{get_redirect(provider).rsplit('/auth',1)[0]}/auth/success?error=oauth", status_code=302)
    ...
    if id_token:
        try:
            ...
            log.warn("Failed to parse id_token", provider=provider, error=str(e))
            userinfo = {}
    ...
    if not userinfo:
        try:
            ...
            log.error("userinfo fetch failed", provider=provider, stack=tb)
            userinfo = {}

    if not userinfo:
        log.warn("No userinfo/claims extracted; aborting oauth callback", provider=provider)
        if provider == "github":
            allowed = ", ".join(sorted(GITHUB_ALLOWED_ORGS)) if GITHUB_ALLOWED_ORGS else None
            return _render_access_denied(
                "Access denied",
                "Unable to fetch GitHub profile information from provider.",
                details="Ensure the OAuth app has correct scopes and the provider is reachable.",
                allowed=allowed,
            )
        return RedirectResponse(url=f"{get_redirect(provider).rsplit('/auth',1)[0]}/auth/success?error=oauth", status_code=302)
    ...
    try:
        tenant = (
            userinfo.get("tid")
            or userinfo.get("tenantId")
            or (id_token and (jwt.decode(id_token, options={"verify_signature": False}).get("tid")))
        )
    except Exception:
    ...
    if not sub or not email:
        log.warn("Essential identity fields missing in userinfo", sub=sub, email=email, provider=provider)
        if provider == "github":
            allowed = ", ".join(sorted(GITHUB_ALLOWED_ORGS)) if GITHUB_ALLOWED_ORGS else None
            return _render_access_denied(
                "Access denied",
                "GitHub identity did not provide an email address.",
                details="A verified email is required. Ensure GitHub user has a verified primary email or grant access to user:email scope.",
                allowed=allowed,
            )
        return RedirectResponse(url=f"{get_redirect(provider).rsplit('/auth',1)[0]}/auth/success?error=oauth", status_code=302)
    ...
    if provider == "github" and GITHUB_ALLOWED_ORGS and access_token:
        try:
            ...
            log.warn("Failed to verify GitHub org membership", provider=provider)
    ...
    log.info("oauth.callback.success", provider=provider, sub=sub, email=email)
    return HTMLResponse(content=html_body)

# --- Keep legacy success, logout and /me endpoints so UI contract remains stable ---
...
async def me(request: Request):
    ...
    if not auth or not auth.lower().startswith("bearer "):
        # Keep old semantics: missing header -> 401 so UI treats unauthenticated
    ...
    try:
        ...
        tb = "".join(traceback.format_exception(type(e), e, e.__traceback__))
        log.error("Unexpected error while decoding token", error=str(e), stack=tb)
        raise HTTPException(status_code=401, detail="Invalid token")
