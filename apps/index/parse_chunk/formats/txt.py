#!/usr/bin/env python3
import os, sys, json, time, logging, hashlib, tempfile, unicodedata, re
...
REQUIRED = ["S3_BUCKET", "S3_RAW_PREFIX", "S3_CHUNKED_PREFIX"]
...
class S3ParquetWriter:
    ...
    def finalize_and_upload(self, out_basename: str) -> Tuple[int, str]:
        ...
        new_md.update({b"schema_version": CHUNKED_SCHEMA_VERSION.encode("utf-8"), b"parser_version": PARSER_VERSION.encode("utf-8"), b"producer": b"txt_parser", b"created_at": datetime.utcnow().isoformat().encode("utf-8")})
...
def parse_file(s3_key: str, manifest: dict) -> dict:
    ...
    try:
        if full_token_count <= max_tokens:
            ...
            for sub in chunker.chunk_document(canonical_full):
                ...
                chunk_id = f"{doc_id}_{chunk_index}"
                chunk_index += 1
                payload = {"document_id": doc_id or "", "file_name": file_name, "chunk_id": chunk_id or "", "chunk_type": "txt_subchunk", "text": chunk_text or "", "token_count": int(token_ct or 0), "figures": "[]", "embedding": None, "file_type": "text/plain", "source_url": s3_path, "page_number": None, "slide_range": None, "row_range": None, "token_range": None, "audio_range": None, "timestamp": datetime.utcnow().isoformat() + "Z", "parser_version": PARSER_VERSION, "tags": manifest.get("tags", []) if isinstance(manifest, dict) else [], "layout_tags": [], "used_ocr": False, "heading_path": [], "headings": [], "line_range": [int(start_line), int(end_line)]}
                writer.write_payload(payload)
    ...
    try:
        ...
        total_ms = int((time.perf_counter() - start_all) * 1000)
        try:
            head = s3.head_object(Bucket=S3_BUCKET, Key=uploaded_s3_key)
            etag = head.get("ETag", "")
            if isinstance(etag, str):
                etag = etag.strip('"')
            size = head.get("ContentLength", 0)
