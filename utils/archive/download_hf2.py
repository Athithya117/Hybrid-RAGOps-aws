...
WORKSPACE_MODELS = os.getenv("WORKSPACE_MODELS", "models")
...
def download_one(repo_id, name, remote_candidates, target_rel, base):
    ...
    for remote in remote_candidates:
        try:
            logger.info("Attempting %s -> %s", remote, target)
            got = hf_hub_download(
                repo_id=repo_id,
                filename=remote,
...
def ensure_model(model):
    ...
    for remote_rel, target_rel in model["items"]:
        ...
        # Also try path without leading folder (some HF repos put files in root)
        if "/" in remote_rel:
        ...
        try:
            for child in onnx_root.iterdir():
                ...
                if child.name.lower() in candidates:
                    try:
                        shutil.rmtree(child)
                        logger.info("Removed stray onnx duplicate %s", child)
                    except Exception as e:
