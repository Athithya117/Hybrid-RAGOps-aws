...
aws s3 ls "s3://e2e-rag-system-42/" --recursive | tail -n 100
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Force sync local data/ with S3 data/.")
...
def compute_concurrency(override: int = 0) -> int:
    ...
    env_val = os.environ.get("CONCURRENT_FILES")
    if env_val:
        try:
            v = int(env_val)
            return max(1, v)
...
def upload_file(local_path: str, rel_path: str, bucket: str, prefix: str, chunksize_mb: int, concurrency: int, region: Optional[str], sha256: Optional[str]):
...
def download_file(bucket: str, s3_key: str, rel_path: str, local_base: str, region: Optional[str], chunksize_mb: int, concurrency: int):
    s3 = get_s3_client(region)
...
def delete_s3_file(bucket: str, s3_key: str, region: Optional[str]):
    s3 = get_s3_client(region)
...
def upload_directory(base_dir: str, bucket: str, prefix: str, concurrency: int, chunksize_mb: int, force: bool, region: Optional[str]):
    ...
    remote_map: Dict[str, Dict[str, object]] = {rel: {"key": key, "size": size, "etag": etag, "metadata_sha256": None} for key, rel, size, etag in remote_entries}
    ...
    if force:
        ...
        with ThreadPoolExecutor(max_workers=concurrency) as executor:
            futures = [executor.submit(delete_s3_file, bucket, key, region) for key in stale_keys]
            for _ in as_completed(futures):
                pass
        remote_entries = list_s3_objects(bucket, prefix, region)
        remote_map = {rel: {"key": key, "size": size, "etag": etag, "metadata_sha256": None} for key, rel, size, etag in remote_entries}
        for rel, info in remote_map.items():
            meta = head_object_metadata(bucket, info["key"], region)
            if meta.get("sha256"):
                remote_map[rel]["metadata_sha256"] = meta.get("sha256")
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        ...
        for local_path, rel in local.items():
            ...
            futures[executor.submit(upload_file, local_path, rel, bucket, prefix, chunksize_mb, concurrency, region, local_sha)] = rel
    ...
    print(f"Upload: succeeded={successes}, skipped={skipped}, failed={len(errors)}")
    for e in errors[:20]:
        print(e)
...
def download_directory(bucket: str, base_dir: str, prefix: str, concurrency: int, force: bool, region: Optional[str], chunksize_mb: int):
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        ...
        for rel, info in remote_map.items():
            ...
            futures[executor.submit(download_file, bucket, key, rel, base_dir, region, chunksize_mb, concurrency)] = rel
    ...
    print(f"Download: succeeded={successes}, skipped={skipped}, failed={len(errors)}")
    for e in errors[:20]:
        print(e)
