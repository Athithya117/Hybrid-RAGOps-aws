...
This is the **global on/off switch** for backups in the indexing pipeline.
When `true`, the pipeline is allowed to decide whether a backup should run based on indexing results.
When `false`, **no backup will ever run**, even if indexing added many points.
Typical usage:

* `true` in normal scheduled indexing runs
* `false` when you plan to run a **separate manual backup job** or during maintenance

---

`MIN_INDEXED_POINTS_FOR_BACKUP=100`
This sets an **absolute minimum change threshold**.
A backup will only be triggered if indexing actually inserted **at least this many new vectors**.
This prevents creating backups for trivial updates (for example, a few documents or retries).
Typical usage:

* Higher values for large, stable corpora
* Lower values (or 1) for small datasets or development environments

---

`MIN_INDEX_DELTA_RATIO_FOR_BACKUP=0.0`
This is a **relative growth threshold**, compared to existing data in Qdrant.
It represents: `indexed_points / existing_points >= MIN_INDEX_DELTA_RATIO_FOR_BACKUP`


When set to `0.0`, this check is **disabled entirely**, and only the absolute threshold applies.
When set to a value like `0.05`, a backup will only run if the collection grew by **at least 5%**.

Important note:
This setting is only reliable if the pipeline can accurately determine how many points existed **before** indexing. If that number is estimated or missing, keeping this at `0.0` is the safest choice.

---

In short:

* `ENABLE_QDRANT_BACKUP` → **permission to run backups**
* `MIN_INDEXED_POINTS_FOR_BACKUP` → **ignore small updates**
* `MIN_INDEX_DELTA_RATIO_FOR_BACKUP` → **ignore small relative growth**

This combination gives you predictable, low-noise backups without risking missed meaningful changes.
