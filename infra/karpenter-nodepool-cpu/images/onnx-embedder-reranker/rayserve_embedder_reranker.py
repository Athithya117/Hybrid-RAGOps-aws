...
# OpenTelemetry instrumentation (use OTEL_* env vars)
...
# Prom client (optional) - only used if PROMETHEUS_DISABLED != 1
...
# OTEL exporter: rely on OTEL env vars (OTEL_EXPORTER_OTLP_TRACES_ENDPOINT etc.)
resource=Resource.create({"service.name": os.getenv("OTEL_SERVICE_NAME","rag8s-onnx")})
tracer_provider = TracerProvider(resource=resource)
# OTLPSpanExporter will read OTLP envs by default when no args are passed (supported by opentelemetry exporter)
...
# lightweight readiness gauges exported via Prom if desired
READY_GAUGE = Gauge("rag8s_initialized","Model initialization state (1=ready,0=not ready)", ["model"]) if not PROM_DISABLED else None
