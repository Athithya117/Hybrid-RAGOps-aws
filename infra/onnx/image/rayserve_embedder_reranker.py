        ...
        if os.path.isdir(p):
            ...
            for root,_,files in os.walk(p):
                if "tokenizer.json" in files or "tokenizer_config.json" in files:
                    ...
                    return root
    logger.warning("No local tokenizer dir found for %s (checked: %s)",repo_id,candidates)
    return None
def resolve_onnx_path(repo_id:str)->Optional[str]:
    ...
    for p in candidates:
        if os.path.isfile(p):
            ...
            return p
    logger.warning("No local ONNX file found for %s (checked: %s)",repo_id,candidates)
    return None
