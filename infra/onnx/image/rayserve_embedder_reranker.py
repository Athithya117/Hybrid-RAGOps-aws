...
if not PROM_DISABLED:
    try:
        start_http_server(PROM_METRICS_PORT)
        logger.info("Prometheus metrics server started on port %s", PROM_METRICS_PORT)
    except Exception:
        logger.exception("Failed to start Prometheus metrics server on %s", PROM_METRICS_PORT)
