...
required_files_check() {
  local base="$1"
  local name="$2"
  for f in "onnx/model_int8.onnx" "config.json" "tokenizer.json" "tokenizer_config.json" "special_tokens_map.json"; do
    local candidate1="$base/onnx/$name/$f"
    local candidate2="$base/onnx/$name/$(basename "$f")"
    if [ ! -f "$candidate1" ] && [ ! -f "$candidate2" ]; then
      error "Missing $f for $name under $base/onnx/$name"
      return 1
    fi
  done
  return 0
...
if [ ! -d "$HOST_MODELS_DIR" ]; then
  error "Host models directory not found: $HOST_MODELS_DIR"
  exit 1
fi
