...
# If we're doing `up` and QDRANT_PRIVATE_IP is provided, store it in pulumi config so code can read cfg.get(...)
if [ "$CMD" = "up" ]; then
  if [ -n "${QDRANT_PRIVATE_IP:-}" ]; then
    pulumi config set qdrant_private_ip "$QDRANT_PRIVATE_IP" --stack "$STACK" >/dev/null 2>&1 || true
  fi
  # Also set MULTI_AZ flag (optional)
  if [ -n "${MULTI_AZ_QDRANT_DEPLOYMENT:-}" ]; then
    pulumi config set multi_az_qdrant_deployment "$MULTI_AZ_QDRANT_DEPLOYMENT" --stack "$STACK" >/dev/null 2>&1 || true
  fi
...
# If we just ran "up", generate pulumi-exports.sh
if [ "$CMD" = "up" ]; then
