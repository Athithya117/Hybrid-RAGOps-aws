#!/usr/bin/env python3
"""
gen_frontend_and_auth.py

Deterministically generate frontend manifests (Namespace, SA+Role+RB, ConfigMap, Secret,
Deployment, Service, optional HPA, Traefik IngressRoute).

- New auth-related envs are injected into the ConfigMap (not secrets) unless they are secrets.
- Uses FRONTEND_URL to compute ingress host and redirect paths.
- Idempotent: writes .inputs_hash and skips if unchanged (SECRET_VALUES excluded).
- Validates with `kubectl --dry-run=client` if available.

Usage:
  export required envs...
  python3 infra/generators/frontend_auth.py --generate
  python3 infra/generators/frontend_auth.py --apply --confirm
  python3 infra/generators/frontend_auth.py --validate
  python3 infra/generators/frontend_auth.py --delete --confirm
"""
...
# -----------------------------
# Config loader (new envs + removed redundant)
# -----------------------------
def load_config() -> Dict[str, Any]:
    ...
    # Core runtime endpoints and removed oauth redirect base (we compute it)
    ...
    # New auth / OIDC envs (injected into ConfigMap)
    cfg["AUTH_MODE"] = os.getenv("AUTH_MODE", "external-id")         # "entra" | "external-id"
    cfg["OIDC_ISSUER"] = os.getenv("OIDC_ISSUER", "")               # optional override, otherwise derived from AZURE_TENANT_ID
    cfg["OIDC_JWKS_URI"] = os.getenv("OIDC_JWKS_URI", "")           # optional override
    cfg["OIDC_AUDIENCE"] = os.getenv("OIDC_AUDIENCE", "")           # required at runtime
    cfg["SPA_CLIENT_ID"] = os.getenv("SPA_CLIENT_ID", "")
    cfg["AZURE_TENANT_ID"] = os.getenv("AZURE_TENANT_ID", "")       # used if OIDC_ISSUER not provided
    cfg["JWKS_REFRESH_INTERVAL_SECONDS"] = int(os.getenv("JWKS_REFRESH_INTERVAL_SECONDS", "900"))

    # CORS runtime flags (app-level)
    ...
    # Secrets (still kept as stringData in k8s Secret)
    cfg["SECRET_NAMES"] = {"JWT_SECRET":"jwt_secret","SESSION_SECRET":"session_secret"}
    secret_values = {}
    for k in cfg["SECRET_NAMES"].keys():
        v = os.getenv(k)
        if v:
            secret_values[k]=v
...
# -----------------------------
# Renderers (inject new auth keys into configmap)
# -----------------------------
...
def render_configmap(cfg: Dict[str, Any]) -> str:
    # deterministic set of keys (non-secret)
    data = {
...
def render_secret(cfg: Dict[str, Any]) -> str:
    if not cfg["SECRET_VALUES"]:
        return ""
    stringData = {}
    for envk,key in cfg["SECRET_NAMES"].items():
        v = cfg["SECRET_VALUES"].get(envk)
        if v:
            stringData[key]=v
    sec = {"apiVersion":"v1","kind":"Secret","metadata":{"name":f"{cfg['SERVICE_NAME']}-secret","namespace":cfg["FRONTEND_NAMESPACE"]},"type":"Opaque","stringData":stringData}
    return safe_yaml(sec)
...
def render_deployment(cfg: Dict[str, Any]) -> str:
    ...
    pod = {
        "apiVersion":"apps/v1","kind":"Deployment","metadata":{"name":f"{cfg['SERVICE_NAME']}-deployment","namespace":cfg["FRONTEND_NAMESPACE"],"labels":labels},
...
def render_ingressroute(cfg: Dict[str, Any], meta: Dict[str, Any]) -> str:
    # Build middleware chain only from meta (low coupling)
    middlewares = []
    ns = meta.get("traefik_namespace") or meta.get("auth_namespace")
    # Append only known middlewares present in meta
    if meta.get("forward_auth_middleware"):
        # forward-auth may be provided by the auth generator; include if present
    ...
    # NOTE: ratelimit_middleware is intentionally optional and only added if meta includes it
    ...
    # Traefik IngressRoute: use websecure entryPoint because Front Door will forward HTTPS to Traefik.
    ir = {
        ...
        "spec":{
            ...
            "routes":[
                {
                    "match":f"Host(`{host}`) && PathPrefix(`/`)",
                    "kind":"Rule",
                    "services":[{"name":f"{cfg['SERVICE_NAME']}-svc","port":cfg["PORT"]}],
                }
            ]
        }
    }
    if middlewares:
        # Traefik expects middlewares as object refs
...
def generate(cfg: Dict[str, Any], dry_run: bool=False):
    ...
    if existing == ihash and not dry_run:
        info("No non-secret changes; skipping generation."); return
...
def apply(cfg: Dict[str, Any], confirm: bool=False):
    ...
    files = [cfg["FILES"]["namespace"], cfg["FILES"]["sa_role"], cfg["FILES"]["configmap"]]
    if cfg["FILES"]["secret"].exists():
        files.append(cfg["FILES"]["secret"])
    files += [cfg["FILES"]["deployment"], cfg["FILES"]["service"]]
    if cfg["HPA_ENABLED"]:
    ...
    info("Applied frontend manifests")
...
def delete_manifests(cfg: Dict[str, Any], confirm: bool=False):
    ...
    info("Deleted frontend manifests")
...
def main():
    ...
    if args.apply:
        generate(cfg, dry_run=args.dry_run); apply(cfg, confirm=args.confirm); return
    if args.validate:
        generate(cfg, dry_run=args.dry_run); validate(cfg); return
    if args.delete:
