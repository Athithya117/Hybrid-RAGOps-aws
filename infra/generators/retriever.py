#!/usr/bin/env python3
"""
retriever.py - safe generator for retrieval/query manifests.

Behavior:
 - Only non-secret manifests are written to disk (namespace, rbac, deployment, service, hpa).
 - Secrets are never written to disk. If USE_AZURE_KEYVAULT=true, an ExternalSecret manifest is emitted (no secret values).
 - If secrets are provided as env vars, they are created/updated in-cluster via kubectl (dry-run->apply pipeline).
 - Deployments reference secrets via secretKeyRef (never inline values).
 - Commands: --generate, --apply, --delete (with --confirm where applicable).
"""
...
def load_config() -> Dict[str, Any]:
    ...
    cfg["SECRET_VALUES"] = {}
    for k in ("QDRANT_API_KEY", "AZURE_STORAGE_ACCOUNT_KEY", "GROQ_API_KEY", "OPENAI_API_KEY"):
        v = os.getenv(k)
        if v:
    ...
    # files (do NOT write secret files to disk)
    m = cfg["MANIFESTS_DIR"]
    cfg["FILES"] = {
        ...
        "hpa": m / "04-hpa.yaml",
        # external secret (safe to write — contains no secret values) when using Key Vault
...
# ExternalSecret manifest (safe to write — no secret values)
def render_external_secret(cfg: Dict[str, Any]) -> Optional[str]:
    ...
    groq_name = os.getenv("AZ_KEYVAULT_GROQ_SECRET", "retriever-groq-api-key")
    openai_name = os.getenv("AZ_KEYVAULT_OPENAI_SECRET", "retriever-openai-api-key")
    if q_name:
    ...
    if groq_name:
        keys.append({"secretKey": "groq_api_key", "remoteRef": {"key": groq_name}})
    if openai_name:
        keys.append({"secretKey": "openai_api_key", "remoteRef": {"key": openai_name}})
...
def create_or_update_secret_in_cluster(cfg: Dict[str, Any], dry_run: bool = False) -> Dict[str, Any]:
    """
    Create/update k8s secret from in-memory env values using kubectl pipeline.
    ...
    for k, v in cfg["SECRET_VALUES"].items():
        ...
        if k == "QDRANT_API_KEY":
            ...
            cmd_parts += ["--from-literal", f"groq_api_key={v}"]
        elif k == "OPENAI_API_KEY":
            cmd_parts += ["--from-literal", f"openai_api_key={v}"]
...
def generate(cfg: Dict[str, Any], dry_run: bool = False) -> None:
    ...
    # render deployment and check no secret literal leaked
    deployment_yaml = render_deployment(cfg)
    leak = detect_secret_leak(deployment_yaml, cfg["SECRET_VALUES"])
    if leak:
        die(f"Secret value for {leak} would be embedded in deployment YAML; refusing to generate. Remove inline secrets.")
    ...
    # write ExternalSecret manifest only if KEYVAULT mode (does NOT contain secret values)
    if cfg["USE_AZURE_KEYVAULT"]:
        ...
        # ensure no secret file exists on disk
        try:
            (cfg["MANIFESTS_DIR"] / "05-secret.yaml").unlink()
        except Exception:
            pass
...
def apply(cfg: Dict[str, Any], confirm: bool = False, dry_run: bool = False) -> None:
    ...
    if secret_res.get("created") is False and secret_res.get("reason") == "no_secrets":
        if not cfg["ALLOW_MISSING_SECRETS"]:
            info("No secrets present. If your app requires secrets (qdrant key, storage key, API keys) provide them as envs or enable KeyVault.")
    ...
    # Build combined YAML stream (namespace, rbac, service, deployment, hpa) and ExternalSecret if present
    parts: List[str] = []
...
def delete(cfg: Dict[str, Any], confirm: bool = False) -> None:
    ...
    if which("kubectl"):
        ...
        # Delete ExternalSecret if created
        if cfg["USE_AZURE_KEYVAULT"]:
            cmds.append(["kubectl", "delete", "externalsecret", f"{cfg['SERVICE_NAME']}-externalsecret", "-n", ns, "--ignore-not-found"])
        # Note: in-cluster secrets created by this script are named <service>-secret; do not delete unless user explicitly does so elsewhere
        for c in cmds:
            try:
