...
def run_kubectl(args:List[str],input_text:Optional[str]=None,timeout:int=60)->int:
    cmd=["kubectl"]+args
    try:
        proc=subprocess.run(cmd,input=(input_text.encode("utf-8") if input_text else None),stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=False,timeout=timeout)
        out=(proc.stdout or b"").decode("utf-8",errors="replace").strip()
        err=(proc.stderr or b"").decode("utf-8",errors="replace").strip()
        if proc.returncode!=0:
            if err: print("kubectl error:",err,file=sys.stderr)
        else:
            if out: print(out)
        return proc.returncode
    except FileNotFoundError:
        print("kubectl not found in PATH",file=sys.stderr);return 127
    except subprocess.TimeoutExpired:
        print("kubectl timed out",file=sys.stderr);return 124
def ensure_kubectl_available()->None:
    ...
    if rc!=0: die("kubectl client not available in PATH")
def ensure_namespace(ns:str)->None:
    ...
    if rc!=0:
        ...
        if rc2!=0: die(f"Failed to create namespace {ns} (kubectl create returned {rc2})")
...
def render_configmap(tunnel_name:str,credentials_path:str,ingress_rules:List[Dict[str,str]],use_token:bool)->Dict[str,Any]:
    ...
    if not use_token:
        if tunnel_name: cfg["tunnel"]=tunnel_name
...
def render_deployment(replicas:int,image:str,use_token:bool,tunnel_name:str,mount_creds:bool,config_key:str="config.yml",creds_key:str="credentials.json")->Dict[str,Any]:
    container={"name":"cloudflared","image":image,"resources":{"requests":{"cpu":"50m","memory":"64Mi"},"limits":{"cpu":"200m","memory":"256Mi"}},"livenessProbe":{"exec":{"command":["cloudflared","--version"]},"initialDelaySeconds":15,"periodSeconds":60,"timeoutSeconds":5},"readinessProbe":{"exec":{"command":["cloudflared","--version"]},"initialDelaySeconds":10,"periodSeconds":10,"timeoutSeconds":3}}
    ...
    if use_token:
        ...
        container["env"]=[{"name":"TUNNEL_TOKEN","valueFrom":{"secretKeyRef":{"name":"cloudflared-token","key":"CLOUDFLARE_TUNNEL_TOKEN"}}},{"name":"CLOUDFLARE_TUNNEL_TOKEN","valueFrom":{"secretKeyRef":{"name":"cloudflared-token","key":"CLOUDFLARE_TUNNEL_TOKEN"}}}]
    else:
        args_list=["tunnel","--no-autoupdate","run"]
        if tunnel_name: args_list.append(tunnel_name)
        container["args"]=args_list
    if mount_creds:
    ...
    if mount_creds: volumes.append({"name":"creds-volume","secret":{"secretName":"cloudflared-tunnel-credentials"}})
    pod_spec["volumes"]=volumes
    deployment={"apiVersion":"apps/v1","kind":"Deployment","metadata":{"name":"cloudflared","namespace":NAMESPACE},"spec":{"replicas":replicas,"selector":{"matchLabels":{"app":"cloudflared"}},"template":{"metadata":{"labels":{"app":"cloudflared"}},"spec":pod_spec}}}
    return deployment
...
def write_manifests(replicas:int,image:str,use_token:bool,tunnel_name:str,embed_secrets:bool,upstream_service:str,hostname:str,credentials_path:str):
    ...
    if hostname: ingress_rules.append({"hostname":hostname,"service":upstream_service})
    ...
    if embed_secrets:
        idx=3
        if CLOUDFLARE_TUNNEL_TOKEN: safe_write(MANIFESTS_DIR/f"{idx:02d}-secret-cloudflared-token.yaml",yaml.safe_dump(render_secret_token(CLOUDFLARE_TUNNEL_TOKEN),sort_keys=False));idx+=1
        if CLOUDFLARE_TUNNEL_CREDENTIALS_B64: safe_write(MANIFESTS_DIR/f"{idx:02d}-secret-cloudflared-credentials.yaml",yaml.safe_dump(render_secret_credentials(CLOUDFLARE_TUNNEL_CREDENTIALS_B64),sort_keys=False));idx+=1
...
def apply_manifests()->None:
    ...
    if rc!=0: die("kubectl apply failed")
...
if __name__=="__main__":
    ...
    if not FRONTEND_HOSTNAME: die("FRONTEND_HOSTNAME must be set")
    ...
    if not (args.write_manifests or args.apply or args.delete): die("One of --write-manifests, --apply, or --delete is required")
    if args.delete:
        if MANIFESTS_DIR.exists(): run_kubectl(["delete","-f",str(MANIFESTS_DIR)])
        info("deleted (where present)");sys.exit(0)
    write_manifests(replicas=replicas,image=image,use_token=use_token,tunnel_name=CLOUDFLARE_TUNNEL_NAME,embed_secrets=args.embed_secrets,upstream_service=UPSTREAM_SERVICE,hostname=FRONTEND_HOSTNAME,credentials_path=credentials_path)
    if args.apply:
        ensure_namespace(NAMESPACE)
        if not args.embed_secrets:
            if CLOUDFLARE_TUNNEL_TOKEN:
                ...
                if rc!=0: die("Failed to apply cloudflared-token secret")
            if CLOUDFLARE_TUNNEL_CREDENTIALS_B64 and not use_token:
                ...
                rc=apply_secret_yaml(sc_yaml,attempts=3)
                if rc!=0: die("Failed to apply cloudflared-tunnel-credentials secret")
        else:
