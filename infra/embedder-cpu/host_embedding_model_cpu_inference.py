...
# Final: works locally (uvicorn) and on Ray cluster (Serve). Robust fallback & port handling.

import os, sys, time, logging, socket, signal
...
CUSTOM_RESOURCE = os.environ.get("CUSTOM_RESOURCE", "")  # set empty to skip requiring
...
# ---------------- FastAPI app ----------------
app = FastAPI(title="ONNX Embedding (serve)")
...
@serve.ingress(app)
class Embedder:
    pass  # embedder uses the FastAPI app via ingress; model initialized per replica on first call
...
def serve_deploy_with_ray(ray_address: str):
    ...
    # build ray_actor_options (include custom resource only if available)
...
def main():
    ...
    # if explicitly empty => force local uvicorn
    if ray_address == "":
    ...
    if ok:
        # deployment succeeded or Serve running; wait for endpoint readiness if possible
        # Check health endpoint on the port that Serve proxy binds (PORT)
        time.sleep(0.5)
        if is_port_open("127.0.0.1", PORT):
            log.info("Serve appears to be listening on port %d. Check http://127.0.0.1:%d/health", PORT, PORT)
            # Block forever so process doesn't exit (Serve is managing replicas)
            signal.pause()
        else:
            # Serve didn't bind port; fall back to uvicorn on a free port
            port = PORT if not is_port_open("127.0.0.1", PORT) else find_free_port(PORT + 1)
            log.info("Serve proxy not listening on %d; falling back to uvicorn on %d", PORT, port)
            run_uvicorn(port)
    else:
        # serve deploy failed — check if proxy already bound and healthy
        if is_port_open("127.0.0.1", PORT):
