...
def create_or_get_registry(resource_group_name: str | Output[str], registry_name: str, sku: str, admin_user_enabled: bool, public_network_access: bool, retention_days: int, location: str):
    _validate_name(registry_name)
    sku_cap = _normalize_sku(sku)
    _validate_retention(int(retention_days))
    try:
        existing = containerregistry.get_registry(resource_group_name=resource_group_name, registry_name=registry_name)
        return {"registry": None, "id": existing.id, "login_server": getattr(existing, "login_server", None), "provisioning_state": getattr(existing, "provisioning_state", None)}
    except Exception:
        ...
        if sku_cap != "Basic":
            registry_args["policies"] = containerregistry.PoliciesArgs(retention_policy=containerregistry.RetentionPolicyArgs(days=int(retention_days), status="Enabled"))
        registry = containerregistry.Registry("acrRegistry", **registry_args)
        return {"registry": registry, "id": registry.id, "login_server": registry.login_server, "provisioning_state": registry.provisioning_state}
